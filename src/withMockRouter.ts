...
export const withMockRouter: decoratorType = (options: {
  meta?: Array<string>;
  params?: Array<string>;
  query?: Array<string>;
}) =>
  makeDecorator({
    ...
    wrapper: (storyFn: StoryFn, context: StoryContext) => {
      ...
      setup((app) => {
        const existingRouter = app.config.globalProperties
        ...
        const forceReload =
          existingRouter &&
          existingRouter.isMocked !== true &&
          existingRoute &&
          existingRoute.isMocked !== true;
        if (forceReload) {
          existingRouter.go(0);
          return;
        } else {
          ...
          app.config.globalProperties.$route = {
            ...
            params: options.params
              ? getFromArgs(context.args, options.params)
              : {},
            query: options.query
              ? getFromArgs(context.args, options.query)
              : {},
          } as MockRoute;
        }
      });
